...
import { mergeDuplicateBooksService } from '../lib/books.service.js';
import { getBookDetailStatsService, readDetailTrendPeriod, readDetailActivityPeriod } from '../lib/books.detail.service.js';
...
router.get('/:id', async (req: Request, res: Response) => {
    try {
        ...
        const trendPeriod = readDetailTrendPeriod(req.query.trendPeriod);
        const activityPeriod = readDetailActivityPeriod(req.query.activityPeriod);
        const user = await getSessionUser(req);

        const { clause: bookLevelClause, params: bookLevelParams } = getLevelFilter(user, 'b', 2);
        const result = await pool.query(
            `SELECT b.*, 
                    c.name AS category_name,
                    s.name AS subject_name,
        ...
        const book = result.rows[0];
                const details = await getBookDetailStatsService(id, book, user, trendPeriod, activityPeriod);
...
router.post('/merge-duplicates', requirePermission('books', 'edit'), async (req: Request, res: Response) => {
    try {
        const result = await mergeDuplicateBooksService();
        res.json(result);
    } catch (error) {
...
// Batch import array from JSON (Scanned Books)
router.post('/batch', requirePermission('books', 'create'), async (req: Request, res: Response) => {
    const client = await pool.connect();
    try {
        const books = req.body.books;
        if (!Array.isArray(books)) {
            res.status(400).json({ error: 'Expected an array of books in req.body.books' });
            return;
        }
        ...
        for (const book of books) {
            const { 
                title, author, isbn, category_id, quantity, level, cover_image, description,
            ...
            if (isbn && isbn.trim()) {
                const check = await client.query('SELECT id FROM books WHERE isbn = $1', [isbn.trim()]);
            ...
            const nextIdResult = await client.query('SELECT get_next_book_id() AS next_id');
            const bookId = nextIdResult.rows[0].next_id;
            const finalQty = quantity ? parseInt(quantity) : 1;

            await client.query(
                `INSERT INTO books (
                    book_id, title, author, isbn, category_id, quantity, available, 
                    cover_image, description, publisher, published_year, 
                    pages, level
                ...
                [
                    ...
                    finalQty, 
                    finalQty,
                    (cover_image || null)?.trim(), 
                    (description || null)?.trim(),
                    (publisher || null)?.trim(),
        ...
        console.error('Error batch importing books:', error);
        res.status(500).json({ error: 'Failed to batch import books' });
    } finally {
        client.release();
    }
});
